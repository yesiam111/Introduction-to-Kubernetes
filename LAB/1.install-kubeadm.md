...
:point_right: **ALL NODES**
   ...
   192.168.1.X node01.<name>.local
   192.168.1.X node02.<name>.local
   192.168.1.X node03.<name>.local
   EOF
   ```

:point_right: **ONLY 1 TIME, ON FIRST CONTROL PLANE**
   ```shell
   kubeadm init \
...
Remove the taints on the control plane so that you can schedule pods on it
   ...
   ```
Add taints on control plane to prevent scheduling non-system pod on it
   ```shell
   kubectl taint nodes [control-plane hostname] node-role.kubernetes.io/control-plane:NoSchedule
   ```

### Join worker nodes
:point_right: **ON NODE(S) WITH ADMIN CREDENTIAL FOR kubectl**
   ```shell
   kubeadm token create --print-join-command
   ```
:point_right: **ON NEW NODE**
   ```shell
